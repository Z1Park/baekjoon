    ...
    public int solution(String[] board) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            if (board[i].charAt(0) != '.' 
                ...
                if (board[i].charAt(0) == 'O') check[0] = true;
                else check[1] = true;
            }
            if (board[0].charAt(i) != '.' 
                && board[0].charAt(i) == board[1].charAt(i)
                && board[1].charAt(i) == board[2].charAt(i)) {
                if (board[0].charAt(i) == 'O') check[0] = true;
                else check[1] = true;
            }
        }
        if (board[0].charAt(0) != '.' 
            ...
            if (board[0].charAt(0) == 'O') check[0] = true;
            else check[1] = true;
        }
        if (board[0].charAt(2) != '.' 
            && board[0].charAt(2) == board[1].charAt(1)
            && board[1].charAt(1) == board[2].charAt(0)) {
            if (board[1].charAt(1) == 'O') check[0] = true;
            else check[1] = true;
        }
        if (count[0] < count[1]) return 0;
        if (Math.abs(count[0] - count[1]) > 1) return 0;
        if (check[0] && check[1]) return 0;
        if (check[0] && count[0] == count[1]) return 0;
        if (check[1] && count[0] > count[1]) return 0;
        return 1;
    }
}